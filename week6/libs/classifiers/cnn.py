...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss += np.float64(0.5) * self.reg * (np.sum(W1*W1) + np.sum(W2*W2) + np.sum(W3*W3))
        dlayer2_out, dW3, db3 = affine_backward(dscores, scores_cache)
        dlayer1_out, dW2, db2 = affine_relu_backward(dlayer2_out, layer2_cache)
        dx, dW1, db1 = conv_relu_pool_backward(dlayer1_out, layer1_cache)
